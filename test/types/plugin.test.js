...
function pluginVirtuals(schema) {
    schema.virtual('fullName').get(function () {
        return "".concat(this.firstName, " ").concat(this.lastName);
    });
    schema.virtual('notDefined').get(function () {
        return 'foobar';
    });
}
function pluginQueryHelpers(schema) {
    schema.query.whereSomething = function () {
        return this.where({ name: 'something' });
    };
}
function pluginMethods(schema) {
    schema.methods.doSomething = function () {
        return 'test';
    };
}
function pluginStatics(schema) {
...
var Foo = mongoose_1.connection.model('Test', testSchema);
function registerPlugin() {
    return __awaiter(this, void 0, void 0, function () {
        var test, test2, test3;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, Foo.findOne().orFail()];
                case 1:
                    test = _a.sent();
                    console.log(test.firstName);
                    console.log(test.fullName);
                    console.log(test.doSomething());
                    return [4 /*yield*/, Foo.findSomething()];
                case 2:
                    test2 = _a.sent();
                    console.log(test2.firstName);
                    console.log(test2.fullName);
                    console.log(test2.doSomething());
                    return [4 /*yield*/, Foo.findOne().whereSomething().orFail()];
                case 3:
                    test3 = _a.sent();
